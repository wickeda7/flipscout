...
export function DealDiscovery() {
  ...
  const {t} = useI18n();
  return hydrated ? <DiscoveryForm /> : <main className="min-w-0 flex-1 p-4 sm:p-8" aria-busy="true">{t("discover.loading")}</main>;
}
function DiscoveryForm() {
  // Mount only in the browser so saved settings initialize once, without
  // persisting server defaults over the user's preferences during hydration.
  const [initialSettings] = useState(() => {
    let saved: string | null = null;
    try { saved = localStorage.getItem(discoveryStorageKey); } catch { /* Storage may be disabled. */ }
    return restoreSettings(window.location.search, saved);
  });
  const {t,locale,setLocale}=useI18n();
  const [retailer,setRetailer]=useState<DiscoveryRetailer>(initialSettings.retailer);
  const [category,setCategory]=useState<DiscoveryCategory>(initialSettings.category);
  const [zip,setZip]=useState(initialSettings.zip);
  const [radiusMiles,setRadiusMiles]=useState(initialSettings.radiusMiles);
  const [sort,setSort]=useState<"default"|"discount"|"price">(initialSettings.sort);
  const [kind,setKind]=useState<DiscoveryKind>(initialSettings.kind);
  ...
  const [share,setShare] = useState<{params:string;url:string;status:"copied"|"copyHelp"}|null>(null);
  const currentParams = searchSettingsParams({retailer,zip,radiusMiles,category,kind,sort});
  const shareState = share?.params === currentParams ? share.status : null;
  ...
  },[retailer,zip,radiusMiles,category,kind,sort]);
  async function shareSearch() {
    const params = currentParams;
    const url=`${window.location.origin}/?${params}`;
    try {await navigator.clipboard.writeText(url);setShare({params,url,status:"copied"});}
    catch {setShare({params,url,status:"copyHelp"});}
  }
  function startSearch(next: DiscoveryQuery) {
    setLoading(true);setFailed(null);setResult(null);
    setQuery({...next,kind:"all"});
  }
  function resetSettings() {
    ...
    setKind(settings.kind);setSort(settings.sort);setQuery(null);setResult(null);setFailed(null);setLoading(false);setShare(null);
  }
  ...
  return <main className="min-w-0 flex-1 p-4 sm:p-8"><div className="mx-auto max-w-6xl">
    ...
    <form className="my-6 grid gap-4 rounded-2xl border border-white/10 bg-white/5 p-5 sm:grid-cols-2 xl:grid-cols-4" onSubmit={e=>{e.preventDefault();startSearch({retailer,category,kind:"all",page:1,zip,radiusMiles});}}>
    ...
    <div className="mb-4 flex flex-wrap gap-2" role="group" aria-label={t("discover.kind")}>
      {(["all","sale","clearance","penny"] as const).map(k=><button key={k} type="button" aria-pressed={kind===k} onClick={()=>chooseKind(k)} className={`rounded-full px-5 py-3 text-sm transition disabled:opacity-40 ${kind===k?"bg-blue-500 text-white":"bg-white/5 text-neutral-300 hover:bg-white/10"}`}>{t(`discover.${k}`)}</button>)}
    ...
    <div className="mb-4 flex flex-wrap items-center gap-3">
      <button type="button" className={button} disabled={!/^\d{5}$/.test(zip)} onClick={()=>void shareSearch()}>{t("discover.share")}</button>
      <button type="button" className={button} disabled={loading} onClick={resetSettings}>{t("discover.reset")}</button>
    ...
    <div aria-live="polite" aria-busy={loading}>
      ...
      {failed&&<div role="alert" className="rounded-xl bg-amber-500/10 p-5 text-amber-200"><p>{t(`discover.${failed}`)}</p><button type="button" className={`${button} mt-4`} onClick={()=>{if(query)startSearch(query);}}>{t("inventory.retry")}</button></div>}
      {result&&<>
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{deals.map(deal=><article key={deal.id} className="flex flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/[0.025]">
          {deal.imageUrl&&<div className="flex h-44 items-center justify-center bg-white p-4"><div className="relative h-full w-full"><Image src={deal.imageUrl} alt="" fill sizes="(min-width: 1280px) 384px, (min-width: 640px) 50vw, 100vw" className="object-contain" /></div></div>}
        ...
        <nav className="my-6 flex items-center justify-between" aria-label={t("find.page")}><button type="button" className={button} disabled={loading||result.query.page===1} onClick={()=>startSearch({...result.query,page:result.query.page-1})}>{t("find.previous")}</button><button type="button" className={button} disabled={loading||!result.hasMore} onClick={()=>startSearch({...result.query,page:result.query.page+1})}>{t("find.next")}</button></nav>
