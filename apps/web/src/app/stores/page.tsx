...
import { RetailerLookup } from '@/components/stores/RetailerLookup';
...
const inputClass =
	'mt-2 w-full rounded-xl border border-white/15 bg-neutral-950 px-3 py-2.5 text-sm text-white focus:outline-none focus:ring-2 focus:ring-emerald-400 disabled:opacity-40';
const buttonClass =
	'rounded-xl border border-white/15 px-4 py-2.5 text-sm font-medium hover:bg-white/10 disabled:opacity-40 disabled:cursor-not-allowed';
function opportunity(s: StoreSummary): StoreOpportunity {
	return {
		...s,
		key: s.id,
		distanceMiles: s.distanceMiles!,
		dealCount: s.activeDealCount,
		profitPerMile: s.distanceMiles ? s.totalPotentialProfit / s.distanceMiles : 0,
		deals: [],
	};
}
...
export default function StoresPage() {
	...
	useEffect(() => {
		...
		setData(null);
		flipScoutApi
			.searchStores(query, controller.signal)
			.then((result) => {
	...
	function search(nextOrigin = origin, nextSort = sort) {
		setQuery({
			...
			radiusMiles: nextOrigin ? radius : undefined,
			sort: nextOrigin ? nextSort : 'name',
			limit: 12,
			offset: 0,
		});
	}
	function applyOrigin(point: GeoLocation | null) {
		...
		const nextSort = point ? 'distance' : 'name';
		setSort(nextSort);
		search(point, nextSort);
	}
	async function locate() {
		...
		try {
			...
			if (version === locationVersion.current)
				setLocationError(error instanceof LocationError ? error.code : 'unavailable');
		} finally {
		...
		<div className='flex min-h-screen bg-neutral-950 text-neutral-100'>
			...
			<main className='min-w-0 flex-1 px-4 py-6 sm:px-6 lg:px-8'>
				<div className='mx-auto max-w-7xl'>
					...
					<section
						className='mb-5 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5'
						aria-label={t('find.origin')}>
						<div className='flex flex-wrap items-center gap-3'>
							<button
								type='button'
								onClick={locate}
								disabled={locating}
								className='flex items-center gap-2 rounded-xl bg-emerald-400 px-4 py-2.5 text-sm font-semibold text-black disabled:opacity-50'>
								<Navigation size={16} />
								{t(locating ? 'find.locating' : 'find.locate')}
							</button>
							<button type='button' onClick={() => applyOrigin(null)} className={buttonClass}>
								{t('find.clear')}
							</button>
						</div>
						...
						<p className='mt-3 text-sm' aria-live='polite'>
							{origin
								? `${t('find.origin')}: ${origin.latitude.toFixed(4)}, ${origin.longitude.toFixed(4)}`
								: t('find.noOrigin')}
						</p>
						{locationError && (
							<p role='alert' className='mt-3 text-sm text-amber-300'>
								{t(`find.${locationError}`)}
							</p>
						)}
						<details className='mt-4'>
							...
							<div className='mt-3 flex flex-wrap items-end gap-3'>
								...
								<button
									...
									onClick={() => {
										try {
											applyOrigin(parseManualLocation(latitude, longitude));
										} catch {
											setLocationError('invalid');
										}
									}}>
									{t('find.apply')}
								</button>
					...
					<form
						...
						className='mb-5 rounded-2xl border border-white/10 bg-white/[0.025] p-5'>
						<div className='grid gap-4 sm:grid-cols-2 lg:grid-cols-3'>
							<label className='text-xs text-neutral-400'>
								{t('find.query')}
								<input value={q} onChange={(e) => setQ(e.target.value)} maxLength={120} className={inputClass} />
							</label>
							<label className='text-xs text-neutral-400'>
								{t('find.retailer')}
								<input
							...
							<label className='text-xs text-neutral-400'>
								...
								<select
									...
									className={inputClass}>
									<option value='name'>{t('find.name')}</option>
							...
							<button
								type='submit'
								className={`${buttonClass} mt-auto flex items-center justify-center gap-2 bg-white text-black hover:bg-neutral-200`}>
								<Search size={16} />
								{t('find.search')}
							</button>
						</div>
					...
					{(data?.dataProvider === 'mock' || data?.stores.some((store) => store.source === 'mock')) && (
						<p className='mb-4 rounded-xl border border-amber-500/20 bg-amber-500/10 p-3 text-sm text-amber-200'>
							{t('find.demo')}
						</p>
					)}
					<div aria-live='polite' aria-busy={loading}>
						...
						{failed && (
							<div role='alert' className='py-8 text-amber-300'>
								<p>{t('find.failed')}</p>
						...
						{data && (
							<>
								<p className='mb-4 text-sm text-neutral-400'>
									{data.total} {t('find.results')}
								</p>
								{data.stores.length === 0 && (
									<p className='rounded-2xl border border-white/10 p-8 text-neutral-400'>{t('find.empty')}</p>
								)}
								<div className='grid gap-4 md:grid-cols-2 xl:grid-cols-3'>
									{data.stores.map((store) => {
										...
										return (
											<article
												key={store.id}
												className='flex flex-col rounded-2xl border border-white/10 bg-white/[0.025] p-5'>
												<p className='text-xs text-emerald-300'>
													{store.retailer} · {store.source}
												</p>
												...
												<p className='mt-4 flex items-center gap-2 text-sm'>
													...
													{store.distanceMiles === null
														? t('find.noDistance')
														: `${store.distanceMiles.toFixed(1)} ${t('find.straight')}`}
												</p>
												<p className='my-4 text-sm text-neutral-400'>
													{store.activeDealCount ? `${t('find.deals')}: ${store.activeDealCount}` : t('find.noDeals')}
												</p>
												<button
													type='button'
													className={`${buttonClass} mb-3`}
													onClick={() => setInventoryStore(store.id)}>
													{t('inventory.open')}
												</button>
												<button
													type='button'
													aria-pressed={added}
													disabled={!origin || (!added && trip.length >= 10)}
													onClick={() => toggleTrip(store)}
													className={`${buttonClass} mt-auto ${added ? 'border-emerald-400 text-emerald-300' : ''}`}>
													{t(added ? 'find.remove' : 'find.add')}
												</button>
											</article>
										);
									})}
								...
								<nav className='my-5 flex items-center justify-between gap-3' aria-label={t('find.page')}>
									<button
										...
										onClick={() =>
											setQuery((current) => ({ ...current, offset: Math.max(0, data.offset - data.limit) }))
										}>
										{t('find.previous')}
									</button>
									<span className='text-sm text-neutral-400'>
										{t('find.page')} {Math.floor(data.offset / data.limit) + 1}
									</span>
									<button
										type='button'
										className={buttonClass}
										disabled={!data.hasMore}
										onClick={() => setQuery((current) => ({ ...current, offset: data.offset + data.limit }))}>
										{t('find.next')}
									</button>
								</nav>
							</>
					...
					{origin && trip.length > 0 && (
						<RoutePlanner
							key={`${origin.latitude},${origin.longitude}:${trip.map((s) => s.id).join('|')}`}
							stores={trip.map(opportunity)}
