    ...
    <div className="flex min-h-screen bg-neutral-950 text-neutral-100">
      ...
      <main className="min-w-0 flex-1 px-4 py-6 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          ...
          {(data?.dataProvider === "mock" || data?.stores.some(store => store.source === "mock")) && <p className="mb-4 rounded-xl border border-amber-500/20 bg-amber-500/10 p-3 text-sm text-amber-200">{t("find.demo")}</p>}
          <div aria-live="polite" aria-busy={loading}>
            {loading && <p className="py-8 text-neutral-400">{t("find.loading")}</p>}
            {failed && <div role="alert" className="py-8 text-amber-300"><p>{t("find.failed")}</p><button type="button" className={`${buttonClass} mt-3`} onClick={() => setAttempt(n => n + 1)}>{t("inventory.retry")}</button></div>}
            {data && <>
